...
const ApplicationCard = ({
  ...
  const [imgSrc, setImgSrc] = useState(
    application.car.imageUrls?.[0] || "/cartypes/suv3.jpg"
  );
...
const contactPerson = userType ==="manager" ? application.renter : application.manager
      ...
      <div className="flex flex-col lg:flex-row  items-start lg:items-center justify-between px-6 md:px-4 py-6 gap-6 lg:gap-4">
          ...
          <div className="flex flex-col justify-between">
            <div>
              ...
              <div className="flex items-center mb-2">
                ...
                <span>{`${application.car.location.city}, ${application.car.location.country}`}</span>
              </div>
            </div>
            <div className="text-xl font-semibold">
              MAD{application.car.pricePerDay}{" "}
        ...
        <div className="flex flex-col justify-between w-full lg:basis-2/12 lg:h-48 py-2 gap-3 lg:gap-0">
          ...
          <div className="flex justify-between">
            ...
            {new Date(application.reservation?.startDate).toLocaleDateString()}
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">End Date:</span>{" "}
            {new Date(application.reservation?.endDate).toLocaleDateString()}
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Next Payment:</span>{" "}
            {new Date(application.reservation?.nextPaymentDate).toLocaleDateString()}
          </div>
        </div>
        ...
        <div className="flex flex-col justify-start gap-5 w-full lg:basis-3/12 lg:h-48 py-2">
          <div>
            <div className="text-lg font-semibold">
              {userType === "manager" ? "Renter" : "Manager"}
            </div>
            <hr className="mt-3" />
          </div>
